...
> 版本：v3.14.0 ｜ 平台：Chrome 浏览器扩展（Manifest V3） ｜ 文档更新日期：2026-07-10
...
| 版本 | 主要变更 |
|---|---|
| 3.14.0 | 修复 confirm() 导致 popup 关闭问题（改用自定义确认弹层）；修复 background.js 并发竞态（重入保护）；修复 init() 无错误处理；修复 setInterval 未清理；修复 togglePin 数据残留；修复 switchGroup 批量模式状态；修复 _toSecids 沪市代码逻辑；修复隐私页远程脚本/占位邮箱/权限不一致 |
| 3.13.0 | 新增东方财富搜索 API 实时搜索补全；添加 `searchapi.eastmoney.com` 主机权限；异步搜索含 300ms 防抖和竞态条件防护；API 失败时降级到本地 4 种匹配 |
| 3.12.0 | 搜索支持 4 种匹配方式（代码前缀/代码包含/拼音/名称/行业）；HOT_STOCKS 扩展至 32 只覆盖常见代码前缀；看板内筛选支持拼音和行业标签 |
| 3.11.0 | 列配置面板优化；列设置返回按钮 CSS 修复 |
| 3.10.x | 批量操作性能优化；East Money null 安全处理；XSS 防护增强 |
| 3.9.x | 分组管理；拖拽排序；置顶功能；新浪 GBK 编码处理 |
